	...
	private ifExpression(): IfExpression {
		...
		if (this.peek().type === TokenType.Else) {
			...
			if (this.peek().type === TokenType.If) {
				...
				const _closeBrace = this.peek(-1);
				else_branch = {
					type: ExpressionType.BlockExpression,
					block: elseBlock,
					span: mergeSpan(_elseToken.span, _closeBrace.span),
				};
			}
		}

		const endToken = else_branch ? else_branch : this.peek(-1);
		const span = mergeSpan(_ifToken.span, endToken.span);
		return ifExpression(condition, then_branch, else_branch, span);
	}
